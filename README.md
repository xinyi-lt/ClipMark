...
6. Click a saved highlight on the page to edit or delete it.
7. Click the ClipMark extension icon to open the popup.
8. Edit notes, copy Markdown, copy plain text, or export a `.md` file from the popup.
...
- `Context menu only`: do not show selection UI; use the extension popup or browser context menu instead.
...
6. 点击页面中已有的高亮，可以编辑备注或删除该条高亮。
7. 点击浏览器工具栏中的 ClipMark 图标，打开插件弹窗。
8. 在弹窗中编辑备注、复制 Markdown、复制普通文本，或导出 `.md` 文件。
...
- `显示小触发器`：选中文字后显示紧凑触发器。
- `仅右键菜单`：不显示选区浮层，只保留插件弹窗和浏览器右键菜单入口。
