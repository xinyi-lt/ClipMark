...
- 用户可见文案（UI 按钮、状态、placeholder、Markdown 导出标签、manifest 描述）必须集中在 `public/_locales/<locale>/messages.json` 管理，代码内通过 `chrome.i18n.getMessage()` 读取，不得硬编码。新增文案时需同时更新 `en` 和 `zh_CN`。
